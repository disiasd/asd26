...
public class DictionarySpeedTest {
    ...
    public static void main(String args[]) {
        ...
        System.gc();
        System.out.println("\n***** Mixed operations Speed Test *****");
        Random rand = new Random(42); 
        start = System.currentTimeMillis();
        for(int i = 0; i < keys.length; i++) {
            int choice = rand.nextInt(3);
            switch (choice) {
                case 0: H.put(keys[i],data[i]); break;
                case 1: H.get(keys[i]);         break;
                case 2: H.remove(keys[i]);      break;
            }                
        }
        end  = System.currentTimeMillis();
        System.out.println("Mixed operations with HashMap:       " + (end-start)/1000.0 +  "s ");
        ...
        for(int i = 0; i < keys.length; i++) {
            int choice = rand.nextInt(3);
            switch (choice) {
                case 0: T.insert(keys[i],data[i]); break;
                case 1: T.search(keys[i]);         break;
                case 2: T.delete(keys[i]);         break;
            }
        }
        end  = System.currentTimeMillis();
        System.out.println("Mixed operations with HashTable:     " + (end-start)/1000.0 +  "s ");
        ...
        for(int i = 0; i < keys.length; i++) {
            int choice = rand.nextInt(3);
            switch (choice) {
                case 0: A.insert(keys[i],data[i]); break;
                case 1: A.search(keys[i]);         break;
                case 2: A.delete(keys[i]);         break;
            }
        }
        end  = System.currentTimeMillis();
        System.out.println("Mixed operations with AVLDictionary: " + (end-start)/1000.0 +  "s ");
    }
}
